...
classdef UR < handle
    ...
    methods
        ...
        function obj = getJPos(obj)
            global URMod        % Bring URMod to function workspace
            obj.JPOS = URMod.getJPos(obj.URX);  % Get joint positions and set result to JOINTS1
        end
        
        % Function to get current joint velocities of a manipulator
        % One user specified argument:
        %  urx = which manipulator (1 or 2)
        function obj = getJVels(obj)
            global URMod        % Bring URMod to function workspace
            vels = URMod.getJVels(obj.URX); % Get velocities from URX1 and set to variable
            obj.JVELS = vels;               % Update VELS1 property
        end
